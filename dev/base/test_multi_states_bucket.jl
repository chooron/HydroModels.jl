...
init_states = ComponentVector(snowpack=100.0, soilwater=1303.0)
...
node_states = ComponentVector(
    snowpack=fill(100.0, node_num), soilwater=fill(1303.0, node_num)
)
...
config = (ptyidx=1:node_num, styidx=1:node_num, timeidx=ts, solver=HydroModels.ManualSolver(mutable=true))
