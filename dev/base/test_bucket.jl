...
# config = (ptyidx=1:10, styidx=1:10, timeidx=ts, interp=LinearInterpolation,
#     solver=HydroModelTools.ODESolver(sensealg=GaussAdjoint(autojacvec=EnzymeVJP())))
# result = bucket_1(node_input, node_pas; initstates=node_states, config...)[end, end, :]

# gradient = Zygote.gradient(pas) do p
#     bucket_1(
#         node_input, node_pas;
#         initstates=node_states, ptyidx=1:10, styidx=1:10, timeidx=ts, interp=LinearInterpolation,
#         solver=HydroModelTools.ODESolver(sensealg=InterpolationAdjoint(autojacvec=EnzymeVJP()))
#     )[end, end, :] |> sum
# end
# # println(gradient)