...
@testset "Multi hydro bucket with state" begin
    ...
    snow_multi_shared = @hydrobucket :snow begin
        ...
        htypes = [1, 2, 2, 2, 1, 3, 3, 2, 3, 2]  # 3 parameter types
    end
    ...
    @testset "Multi-node with independent parameters" begin
        ...
        single_config = create_test_config(timeidx = ts)
        single_output = snow_single(input, single_params, single_config; initstates = ComponentVector(snowpack = 0.0))
...
@testset "Multi hydro bucket without state" begin
    ...
    snow_multi_shared = @hydrobucket :snow begin
        ...
        htypes = [1, 2, 3, 3, 2, 1, 2, 1, 3, 2]  # 3 parameter types
    end
    ...
    @testset "Multi-node with independent parameters" begin
        ...
        single_config = create_test_config(timeidx = ts)
        single_output = snow_single(input, single_params, single_config; initstates = node_states)
        target_output = permutedims(
            reduce((m1, m2) -> cat(m1, m2, dims=3), repeat([single_output], NUM_NODES)),
            (1, 3, 2)
        )
