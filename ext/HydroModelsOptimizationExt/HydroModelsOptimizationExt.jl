...
function SciMLBase.OptimizationProblem(
    ...
    config = (solver=solver, interpolator=interp, timeidx=timeidx, initstates=default_initstates)

    function objective(p, c)
        output = component(input, p, c; timeidx=c.timeidx, initstates=c.initstates)
        loss_func(target[warm_up:end], output[end, warm_up:end])
    end

    optfunc = OptimizationFunction(objective, adtype)
    prob = OptimizationProblem(optfunc, default_pas, config; prob_kwargs...)
