"""
    SingleHydroBucket{N,S} <: AbstractBucket
...
# Constructor
```julia
SingleHydroBucket(; name=nothing, fluxes, dfluxes=StateFlux[])
```
...
"""
struct SingleHydroBucket{N,S} <: AbstractHydroBucket
    ...
    function SingleHydroBucket(;
        ...
        return new{bucket_name,!isempty(states)}(fluxes, dfluxes, flux_func, ode_func, infos)
    end
end

struct MultiHydroBucket{N,S} <: AbstractBucket
    "Vector of flux functions describing hydrological processes."
    fluxes::Vector{<:AbstractHydroFlux}
    "Vector of state derivative functions for ODE calculations."
    dfluxes::Vector{<:AbstractStateFlux}
    ...
    function MultiHydroBucket(;
        name::Optional{Symbol}=nothing,
        fluxes::Vector{<:AbstractHydroFlux},
        dfluxes::Vector{<:AbstractStateFlux}=StateFlux[],
        nmul::Int=2
    )
        @assert nmul > 1 "Number of nodes must be greater than 1."
        inputs, outputs, states = get_vars(fluxes, dfluxes)
        params, nns = reduce(union, get_params.(vcat(fluxes, dfluxes))), reduce(union, get_nns.(fluxes))
        infos = (; inputs=inputs, outputs=outputs, states=states, params=params, nns=nns)
        flux_func, ode_func = build_multi_bucket_func(fluxes, dfluxes, infos)
        bucket_name = isnothing(name) ? Symbol("##bucket#", hash(infos)) : name
        return new{bucket_name,!isempty(states)}(fluxes, dfluxes, flux_func, ode_func, nmul, infos)
    end
end

"""
    @SingleHydroBucket name begin
        fluxes = begin
            ...
        end
        dfluxes = begin
...
Creates a SingleHydroBucket with the specified name, fluxes, and dfluxes.
...
# Example
```julia
@SingleHydroBucket :bucket1 begin
    fluxes = [flux1, flux2]
...
macro hydrobucket(args...)
    ...
    expr = if isnothing(nmul_val)
        push!(kwargs, :($:(nmul=$nmul_val)))
        :(MultiHydroBucket($(kwargs...)))
    else
        :(SingleHydroBucket($(kwargs...)))
    end
    return esc(expr)
end

"""
    multiply(bucket::SingleHydroBucket, nmul=10)::MultiHydroBucket

Multiplies a SingleHydroBucket by a given number of nodes.

# Arguments
- `bucket`: SingleHydroBucket to be multiplied
- `nmul`: Number of nodes to multiply by

# Returns
- MultiHydroBucket with the same fluxes and dfluxes as the input bucket, but with the specified number of nodes
"""
function multiply(bucket::SingleHydroBucket, nmul=10)::MultiHydroBucket
    MultiHydroBucket(name=bucket.name, fluxes=bucket.fluxes, dfluxes=bucket.dfluxes, nmul=nmul)
end

"""
    (bucket::SingleHydroBucket{N,S})(input::AbstractArray{T,2}, params::ComponentVector; kwargs...)
    (bucket::MultiHydroBucket{N,S})(input::AbstractArray{T,3}, params::ComponentVector; kwargs...)

Evaluate the SingleHydroBucket model with input data and parameters.
...
"""
function (ele::SingleHydroBucket{N,true})(input::AbstractArray{T,2}, params::ComponentVector; kwargs...) where {T,N}
    #* get kwargs
...
function (ele::MultiHydroBucket{N,true})(input::AbstractArray{T,3}, params::ComponentVector; kwargs...) where {T,N}
...
function (ele::SingleHydroBucket{N,false})(input::AbstractArray{T,2}, params::ComponentVector; kwargs...) where {T,N}
    stack(ele.flux_func(input, nothing, params), dims=1)
end

function (ele::MultiHydroBucket{N,false})(input::AbstractArray{T,3}, params::ComponentVector; kwargs...) where {T,N}
