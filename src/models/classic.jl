...
bucket3 = @hydrobucket :bucket3 begin
    fluxes = begin
        @hydroflux FS1 ~ step_func(SS1MAX - SS1) * CFS1 * exp(-XFS1 * SS1/SS1MAX)
        @hydroflux ETAS1 ~ step_func(SS1 - FCCS1 * SS1MAX) * FF * Ep + (1 - FF) * SS1/SS1MAX * Ep + FF * SS1/FCCS1 * SS1MAX * Ep
...
model = @hydromodel :classic begin
    bucket1
    bucket2
    bucket3
    uh_1
